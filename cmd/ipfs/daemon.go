...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) (_err error) {
	...
	fmt.Println("@lry: cctx.ConfigRoot " + cctx.ConfigRoot)
	switch err {
	default:
		return err
	...
	cfgb, err := repo.Config()
	if err != nil {
		fmt.Println("repo config read error")
		return err
	}
	var removed []string
	...
	if node.DHT.LAN.Mode() == 0 || node.DHT.LAN.Mode() == 1 {
		fmt.Println("@lry LAN modeClient")
	} else {
		fmt.Println("@lry LAN modeServer")
	}

	if node.DHT.WAN.Mode() == 0 || node.DHT.WAN.Mode() == 1 {
		fmt.Println("@lry WAN modeClient")
	} else {
		fmt.Println("@lry WAN modeServer")
	}
	...
	for str := range allKeys {
		...
		strs = append(strs, str.Hash().B58String())
	...
	for _, str := range strs {
		if node.DHT.WAN.GetBF().HasString(str) {
			fmt.Printf("@lry bloom %s yes \n: ", str)
		} else {
			fmt.Printf("@lry bloom %s no \n: ", str)
		}
	}

fmt.Printf("@lry: self = %s\n", node.Identity)
