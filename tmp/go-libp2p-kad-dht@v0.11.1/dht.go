...
type IpfsDHT struct {
	...
	//BF list
	// BFList []bloom.BloomFilter
	BFList map[peer.ID]bloom.BloomFilter
...
// 开启数据中心搜索
func (dht *IpfsDHT) OpenCenter(ip string) {
	dht.enableDataCenter = true
	dht.dataCenterIP = ip
}

// 关闭数据中心搜索
func (dht *IpfsDHT) CloseCenter() {
	dht.enableDataCenter = false
}

// 测试HTTP请求
func (dht *IpfsDHT) FindHttp(ip string, key string) bool {
	reqURL := "http://" + ip + ":5001/api/v0/dht/findprovs?arg=" + key
	fmt.Printf("@lry: url = %s \n", reqURL)
...
// 设置DHT中的BF
func (dht *IpfsDHT) SetBF(bf bloom.BloomFilter) {
	dht.BF = bf
}

// 更改enableBF
func (dht *IpfsDHT) SetenableBF(enable bool) {
	dht.enableBF = enable
}

// 获取DHT中的BF
func (dht *IpfsDHT) GetBF() bloom.BloomFilter {
	return dht.BF
}

func (dht *IpfsDHT) PrintRoutingTable() {
	fmt.Println("test routing table print")
	dht.routingTable.Print()
}

// 收集附近节点的BF，填充BF list
func (dht *IpfsDHT) GenBFList(ctx context.Context, delay int64) int {
	dht.BFList = make(map[peer.ID]bloom.BloomFilter)
	count := 0
	// ps := dht.routingTable.ListPeers()
	for _, p := range dht.host.Network().Peers() {
		//向这个节点请求BF
		fmt.Printf("@lry: peer = %s \n", p)
		// 条件
		if dht.peerstore.LatencyEWMA(p).Milliseconds() > delay {
			fmt.Printf("@lry: peer %s delay is %s to far \n", p, dht.peerstore.LatencyEWMA(p).String())
			continue
		}
		pmes := pb.NewMessage(pb.Message_GET_BF, nil, 0)
		// rpms为返回消息
		rpmes, err := dht.sendRequest(ctx, p, pmes)
		if err != nil {
			fmt.Printf("@lry: peer %s return BF failed", p)
			continue
		}
		// 构建BF
		if rpmes.GetBF() == nil {
			fmt.Printf("@lry: get peer %s BF error \n", p)
			continue
		}
		var filter bloom.BloomFilter = bloom.ConstructMemoryBloomFilter(rpmes.GetBF())

		dht.BFList[p] = filter
		count++

	}
	fmt.Printf("@lry: bflist len = %d \n", len(dht.BFList))
	return count
}
