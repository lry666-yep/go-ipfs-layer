...
func (dht *IpfsDHT) handlerForMsgType(t pb.Message_MessageType) dhtHandler {
	...
	case pb.Message_GET_BF:
		return dht.handleGetBF
	}
...
func (dht *IpfsDHT) handleFindPeer(ctx context.Context, from peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	...
	if targetPid == dht.self {
		...
		closest = dht.betterPeersToQuery(pmes, from, 3)
...
func (dht *IpfsDHT) handleGetProviders(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	c, err := cid.Parse(pmes.GetKey())
	if err != nil {
		return nil, fmt.Errorf("handleGetProviders key parse err")
	}
	fmt.Printf("@lry: handlerGetProv key %s \n", c)
	key := pmes.GetKey()
	if len(key) > 80 {
	...
	// setup providers
	pds := dht.ProviderManager.GetProviders(ctx, key)
	if pds != nil {
		for _, p := range pds {
			providers = append(providers, p)
		}
	}
	// 请求BF查看是否有节点保存，加入到resp.ProviderPeers中
	if dht.enableBF {
		for p, bf := range dht.BFList {
			if bf.HasString(c.String()) {
				fmt.Printf("@lry: handler getProv BF hit!")
				providers = append(providers, p)
			}
		}
	}
...
// 新增处理节点BF获取请求
func (dht *IpfsDHT) handleGetBF(ctx context.Context, from peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	resp := pb.NewMessage(pmes.GetType(), nil, pmes.GetClusterLevel())
	//dht.BF 从bitset中提取int64[] 放入 resp.BF

	resp.BF = dht.BF.GetBitSet()
